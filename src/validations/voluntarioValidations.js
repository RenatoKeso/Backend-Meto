...
const passwordRule = Joi.string().min(8).max(72).messages({
...
const emailRule = Joi.string().trim().email().max(150).messages({
...
const edadRule = Joi.number().integer().min(18).max(120).messages({
...
const contactoRule = Joi.string()
  ...
  .messages({
    ...
    "string.pattern.base":
      "El contacto debe contener solo numeros y opcionalmente + al inicio",
    "string.max": "El contacto no puede exceder 15 caracteres",
  });

const clasificacionRule = Joi.string().trim().max(15).messages({
...
const nivelCapacidadRule = Joi.string()
  .valid("baja", "media", "alta")
  .messages({
    "any.only": "El nivel debe ser: baja, media o alta",
...
const capacidadFisicaSchema = Joi.object({
  movilidad: nivelCapacidadRule.optional(),
  resistencia_fisica: nivelCapacidadRule.optional(),
  capacidad_carga: nivelCapacidadRule.optional(),
  otras_habilidades: Joi.string()
    .trim()
    .max(500)
    .allow("", null)
    .optional()
    .messages({
      "string.max": "Las otras habilidades no pueden exceder 500 caracteres",
    }),
})
  .min(1)
  .messages({
    "object.min":
      "Debes enviar al menos un campo para actualizar (movilidad, resistencia_fisica, capacidad_carga u otras_habilidades)",
  });
